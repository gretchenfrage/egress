 ...
 *
 * <p>
 * After construction a GeomShaderProgram can be used to draw {@link Mesh}. To make the GPU use a specific GeomShaderProgram the programs
 * {@link GeomShaderProgram#begin()} method must be used which effectively binds the program.
 * </p>
 *
 * <p>
 * When a GeomShaderProgram is bound one can set uniforms, vertex attributes and attributes as needed via the respective methods.
 * </p>
 *
 * <p>
 * A GeomShaderProgram can be unbound with a call to {@link GeomShaderProgram#end()}
 * </p>
 *
 * <p>
 * A GeomShaderProgram must be disposed via a call to {@link GeomShaderProgram#dispose()} when it is no longer needed
 * </p>
 *
 * <p>
 * GeomShaderPrograms are managed. In case the OpenGL context is lost all shaders get invalidated and have to be reloaded. This
 * happens on Android when a user switches to another application or receives an incoming call. Managed GeomShaderPrograms are
...
public class GeomShaderProgram extends ShaderProgram {
    ...
    private final static ObjectMap<Application, Array<GeomShaderProgram>> shaders = new ObjectMap<Application, Array<GeomShaderProgram>>();
    ...
    private static String findVertexShader(Array<ShaderPart> parts) {
        for (ShaderPart part : parts) {
            if (part.stage.equals(ShaderStage.vertex)) {
                return part.source;
            }
        }
    ...
    private static String findFragmentShader(Array<ShaderPart> parts) {
        for (ShaderPart part : parts) {
            if (part.stage.equals(ShaderStage.fragment)) {
                return part.source;
            }
        }
        throw new NoSuchElementException();
    ...
    /** Constructs a new GeomShaderProgram and immediately compiles it.
     * <p>
     ...
     * @param parts shaders
     */
    public GeomShaderProgram(Array<ShaderPart> parts) {
        super(findVertexShader(parts), findFragmentShader(parts));
    ...
    public GeomShaderProgram(ShaderPart ...parts) {
    ...
    public GeomShaderProgram (String vertexShader, String fragmentShader) {
        this(new ShaderPart(ShaderStage.vertex, vertexShader), new ShaderPart(ShaderStage.fragment, fragmentShader));
    }

    public GeomShaderProgram (FileHandle vertexShader, FileHandle fragmentShader) {
...
// }
    ...
    /** @return whether this GeomShaderProgram compiled successfully. */
    ...
    /** Sets the uniform with the given name. The {@link GeomShaderProgram} must be bound for this to work.
     *
    ...
    /** Sets the uniform matrix with the given name. The {@link GeomShaderProgram} must be bound for this to work.
     *
     * @param name the name of the uniform
    ...
    /** Sets an array of uniform matrices with the given name. The {@link GeomShaderProgram} must be bound for this to work.
     *
     * @param name the name of the uniform
    ...
    /** Sets the vertex attribute with the given name. The {@link GeomShaderProgram} must be bound for this to work.
     *
    ...
    private void addManagedShader (Application app, GeomShaderProgram GeomShaderProgram) {
        Array<GeomShaderProgram> managedResources = shaders.get(app);
        if (managedResources == null) managedResources = new Array<GeomShaderProgram>();
        managedResources.add(GeomShaderProgram);
        shaders.put(app, managedResources);
    }

    /** Invalidates all shaders so the next time they are used new handles are generated
     * @param app */
    public static void invalidateAllGeomShaderPrograms (Application app) {
        if (Gdx.gl20 == null) return;

        Array<GeomShaderProgram> shaderArray = shaders.get(app);
    ...
    public static void clearAllGeomShaderPrograms (Application app) {
        shaders.remove(app);
    }
    ...
    public static int getNumManagedGeomShaderPrograms () {
