...
class FGraphicsCloudTest extends GameState {
  ...
  override def onEnter(driver: GameDriver): Unit = {
    ...
    // make the globals
    globals = GlobalRenderData(Origin, North, Origin, 1, V4F(1, 1, 1, 1), 90)
  ...
  override def render(): Unit = {
    processor.update()
    globals = globals.copy(camPos = V3F(renderer.cam.position), camDir = V3F(renderer.cam.direction), fov = 90)
