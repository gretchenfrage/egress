...
class InfWorld(
  ...
  def --(ps: Seq[V3I]): InfWorld = {
    new InfWorld(time, res, chunks -- ps, state1 -- ps, state2 -- ps, state3 -- ps)
  }

  def -(p: V3I): InfWorld = {
    new InfWorld(time, res, chunks - p, state1 - p, state2 - p, state3 - p)
  }

  /**
    * Add those chunks to the world.
    */
  def ++(cs: Seq[Chunk]): InfWorld = {
    val removed = this -- cs.map(_.pos)
    val grouped = cs.groupBy(_.terrain.terrainType).mapValues(_.map(_.pos)).withDefault(_ => Seq.empty)
    new InfWorld(time, res,
  ...
  /**
    ...
    * equals frequency.
    */
  def integrate(events: Map[V3I, Seq[ChunkEvent]], mod: Int, freq: V3I): InfWorld = {
  ...
  /**
    ...
    * parallel.
    */
  def integrate(events: Seq[ChunkEvent]): InfWorld = {
  ...
  def incrTime: InfWorld = {
    new InfWorld(time + 1, res, chunks, state1, state2, state3)
  }
...
class Infinitum(res: Int, save: AsyncSave, dt: Float) {
  ...
  @volatile private var history = SortedMap(0L -> new InfWorld(0, res, Map.empty, Set.empty, Set.empty, Set.empty))
  ...
  def apply(t: Long): Option[InfWorld] = history.get(t)
