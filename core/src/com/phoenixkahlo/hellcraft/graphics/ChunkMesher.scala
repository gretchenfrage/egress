...
import com.phoenixkahlo.hellcraft.math.{Origin, Quad, Tri, V3I}
...
class ChunkMesher(chunk: Chunk, meshable: Meshable) {
  ...
  val mesh = new DisposableParamCache[(World, ResourcePack), Renderable]({ case (world, pack) => {
    ...
    for (v: V3I <- meshable.vertMap) {
      ...
      val texU = vert.p.x % (u2 - u1) + u1
      val texV = vert.p.z % (v2 - v1) + v1

      vertData.append(
        vert.p.x, vert.p.y, vert.p.z, Color.WHITE.toFloatBits, texU, texV, vert.n.x, vert.n.y, vert.n.z
      )
    }
