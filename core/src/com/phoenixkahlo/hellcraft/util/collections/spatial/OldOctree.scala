...
sealed trait OldOctree[+E] extends Map[V3F, E] {
  override def +[V1 >: E](kv: (V3F, V1)): OldOctree[V1]

  override def -(key: V3F): OldOctree[E]
...
/**
  ...
  */
@deprecated("use SpatialHashMap instead", "N/A")
private case class EmptyOctree(center: V3F, range: Float) extends OldOctree[Nothing] {
  override def +[V1 >: Nothing](kv: (V3F, V1)): OldOctree[V1] = OctreeLeaf(center, range, kv)

  override def get(key: V3F): Option[Nothing] = None

  override def iterator: Iterator[(V3F, Nothing)] = Iterator.empty

  override def -(key: V3F): OldOctree[Nothing] = this
...
/**
  ...
  */
@deprecated("use SpatialHashMap instead", "N/A")
private case class OctreeLeaf[+E](center: V3F, range: Float, elem: (V3F, E)) extends OldOctree[E] {
  override def +[V1 >: E](kv: (V3F, V1)): OldOctree[V1] = {
    if (kv._1 == elem._1) OctreeLeaf(center, range, kv)
    else {
      val children = OldSigns().map(sign => sign -> EmptyOctree(center + (sign * range / 2), range / 2)).toMap
...
@deprecated("use SpatialHashMap instead", "N/A")
private case class OctreeBranch[+E](center: V3F, range: Float, children: Map[V3I, OldOctree[E]]) extends OldOctree[E] {
  override def +[V1 >: E](kv: (V3F, V1)): OldOctree[V1] = {
    val (key, value) = kv
  ...
  override def closest(point: V3F): Option[(V3F, E)] = {
    OldSigns().flatMap(children(_).closest(point)).sortBy(_._1 dist point).headOption
...
object OctreeTest extends App {
  ...
  var octree: OldOctree[Unit] = EmptyOctree(Origin, Float.MaxValue)
  /*
  for (n <- 1 to 100) {
    octree += ((V3I(n, n, -n), n))
  }
