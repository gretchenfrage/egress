...
object Raytrace {
  ...
  def hit(pos: V3F, dir: V3F, world: World, range: Float = 4): Option[V3I] =
    apply(pos, dir).takeWhile(v => (v + V3F(0.5f, 0.5f, 0.5f) dist pos) <= range)
      .takeWhile(world.blockAt(_) isDefined).find(world.blockAt(_).get != Air)

  def place(pos: V3F, dir: V3F, world: World, range: Float = 4): Option[V3I] =
    apply(pos, dir).takeWhile(v => (v + V3F(0.5f, 0.5f, 0.5f) dist pos) <= range)
      .takeWhile(world.blockAt(_).exists(_ == Air)).lastOption
