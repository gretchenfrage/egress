...
@CarboniteFields
case class TerrainSoup(pos: V3I, verts: OptionField[TerrainSoup.Vert], indices: Seq[NodeTypeID], indexToVert: Seq[V3I], vertToIndex: ShortField)
object TerrainSoup {
  ...
  if (Terrain.canComplete(terrain.pos, world)) {
    ...
    val verts = OptionField[TerrainSoup.Vert](world.resVec + Ones, v => {
      ...
      if (corners.exists(_.id < 0)) {
        ...
        corners.find(_.id > 0).map(mat => Vert((offset + v + V3F(0.5f, 0.5f, 0.5f)) / world.res * 16, mat.asInstanceOf[Material]))
      } else {
        ...
        if (isopoints isEmpty) None
          ...
          Some(TerrainSoup.Vert(vertPos, mat.asInstanceOf[Material]))
        }
      }
    })
    ...
    if (canComplete(world)) {
      ...
      val verts = OptionField[TerrainSoup.Vert](world.resVec + Ones, v => {
        ...
        if (corners.exists(_.id < 0)) {
          // if one of the corners is a block, we must snap to the center
          // but if none of the corners are terrain, we shouldn't place a vertex at all
          corners.find(_.id > 0).map(mat => TerrainSoup.Vert((offset + v + V3F(0.5f, 0.5f, 0.5f)) / world.res * 16, mat))
        } else {
          // otherwise, we can do surface nets as normal
          ...
          if (isopoints isEmpty) None
            ...
            Some(TerrainSoup.Vert(vertPos, mat))
          }
        }
      })
...
case class CompleteTerrain(pos: V3I, grid: IDField[TerrainUnit],
                           verts: OptionField[TerrainSoup.Vert], indices: Seq[NodeTypeID], indexToVert: Seq[V3I], vertToIndex: ShortField,
...
object CompleteTerrain {
  @CarboniteFields case class TerrainSoup.Vert(pos: V3F, mat: TerrainUnit)
